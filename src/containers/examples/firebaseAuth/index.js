  ...
  componentDidMount() {
    ...
    if (this.props.authUser !== null) {
      this.props.history.push(
        appURLs.examples.components.wedevlopUI.stripeElements.home
      );
    }
  }
        ...
        <div style={styleObjects.examplesContainer}>
          ...
          {this.props.userDoc !== undefined && this.props.userDoc !== null && (
            <div>
              ...
              <Body text={`Created: ${new Date(this.props.userDoc.created_at.seconds * 1000).toLocaleDateString("en-US")}`} />
            </div>
          )}

          <Subheading text="FIREBASE AUTH USER" />
          <Body text="Preview" />
          {this.props.firebaseUser !== undefined && this.props.firebaseUser !== null && (
            <div>
              <Body text={`UID: ${this.props.firebaseUser.uid}`} />
              <Body text={`Email: ${this.props.firebaseUser.email}`} />
              <Body text={`Email Verified: ${this.props.firebaseUser.emailVerified.toString()}`} />
              <Body text={`Phone Number: ${this.props.firebaseUser.phoneNumber}`} />
              <Body text={`Display Name: ${this.props.firebaseUser.displayName}`} />
              ...
              {this.props.firebaseUser.metadata !== undefined
                ...
                && <Body text={`Creation: ${this.props.firebaseUser.metadata.creationTime}`} />}
              {this.props.firebaseUser.metadata !== undefined
                && this.props.firebaseUser.metadata !== null
                && <Body text={`Last Sign In: ${this.props.firebaseUser.metadata.lastSignInTime}`} />}
            </div>
          )}
          ...
          <AuthResetPass
            ...
            onSubmit={async data => {
              ...
              if (this.props.authError === null) {
                ...
                alert("An email with a link to reset your password has been sent to your email inbox.")
              }
            }}
            authError={this.props.authError}
          />
